...
// @access  Public
router.get("/test", (req, res) => res.json({ msg: "Nice! Your test connection worked." }));
...
router.post(
  ...
  (req, res) => {
    ...
    // SPLIT INTO AN ARRAY
    if (typeof req.body.skill !== undefined) profileFields.skills = req.body.skills.split(',');
    // SOCIAL
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        } else { // if no profile then create one
